/**
 * Created by snowden.xu on 2017/3/22.
 */
...
/* 提交通用 */
{
    ...
    function submitPlan() {
        ...
        for(var i=0;i<rows.length;i++){
            var row={};
            row.approvalProcessUser=row.UserName
            row.approvalProcessRole=row.RoleName
            row.approvalProcessMode=row.Mode;
            userList.push(row);
        }
        if(userList.length == ""){
            tms.alert('请选择主审人');
        ...
        tms.services.approvalSubmit({
            requestBody:{
                "approvalSubmitUser": userInfo.name,
                "userList":rows,
                "referencedType": "workperiodsheet",
                "referencedId": 1,
